...
class SessionTest {
    ...
    void shouldStoreCurrentCollection() {
        ...
        collection.add(Bus.builder().routeNumber(1).model("A").mileage(100).build());
    ...
    void shouldAllowOverwritingLastResult() {
        List<Bus> first = List.of(
                Bus.builder().routeNumber(1).model("A").mileage(100).build()
        );
        List<Bus> second = List.of(
                Bus.builder().routeNumber(2).model("B").mileage(200).build()
        );
